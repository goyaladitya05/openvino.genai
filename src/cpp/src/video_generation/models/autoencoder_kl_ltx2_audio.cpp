...
AutoencoderKLLTX2Audio::AutoencoderKLLTX2Audio(const std::filesystem::path& vae_decoder_path)
...
AutoencoderKLLTX2Audio::AutoencoderKLLTX2Audio(const std::filesystem::path& vae_decoder_path,
    ...
    : AutoencoderKLLTX2Audio(vae_decoder_path) {
...
AutoencoderKLLTX2Audio& AutoencoderKLLTX2Audio::compile(const std::string& device, const ov::AnyMap& properties) {
    ...
    ov::CompiledModel decoder_compiled_model = utils::singleton_core().compile_model(m_decoder_model, device, *filtered_properties);
    ov::genai::utils::print_compiled_model_properties(decoder_compiled_model, "Auto encoder KL LTX2 audio decoder model");
...
AutoencoderKLLTX2Audio& AutoencoderKLLTX2Audio::reshape(int64_t batch_size) {
    ...
    for (int64_t i = 1; i < input_shape.rank().get_length(); ++i) {
        input_shape[i] = -1;
    }
